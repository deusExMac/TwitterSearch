...
In conclusion: set at least the value of essentialBearer to the value of the essential bearer token you received from your Twitter developer account. If you also habe an acdemic bearer token, set the token as value for the setting academicBearer in the configuration file. Set also the value of targetArchive to the value depending on the type of search you want to conduct (recent or historic). Hence, the minimum settings that need to be set in the configuration file are:  **`essentialBearer`** and **`targetArchive`** .
